...
*//*
 *//* Future<Map<int, List<Location>>> generateItinerary(String tripRoomId, int initialStartHour) async {
    try {
      final userPosition = await determinePosition();
      final locations = await getWishlistItems(tripRoomId); // Assuming you are getting wishlist items for the tripRoomId
      sortLocationsByDistance(locations, userPosition);

      Map<int, List<Location>> dailyItinerary = {};
      int currentDay = 1;
      DateTime currentTime = DateTime(DateTime.now().year, DateTime.now().month, DateTime.now().day, initialStartHour);

      while (locations.isNotEmpty) {
        List<Location> dailyLocations = [];
        List<Location> filteredLocations = filterLocationsByTime(locations, currentTime);

        for (var location in filteredLocations) {
          final nextStartTime = _getNextStartTime(currentTime, location.operatingHour!);
          final approximateTimeMinutes = ((location.approximateTime ?? 2) * 60).ceil();
          final endTime = nextStartTime.add(Duration(minutes: approximateTimeMinutes));

          if (dailyLocations.isEmpty || endTime.isBefore(DateTime(currentTime.year, currentTime.month, currentTime.day + 1, 0))) {
            location.endTime = endTime;
            dailyLocations.add(location);
            currentTime = endTime;
            locations.remove(location);
          }
        }

        dailyItinerary[currentDay] = dailyLocations;
        currentDay++;
        currentTime = DateTime(currentTime.year, currentTime.month, currentTime.day + 1, initialStartHour);
      }
